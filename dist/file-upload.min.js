...
(function(window){var Backbone=window.Backbone,_=window._,$=window.$,file_upload={Views:{}};file_upload.Model=Backbone.Model.extend({});file_upload.Collection=Backbone.Collection.extend({model:file_upload.Model,initialize:function(options){options=options||{};this.url=options.url||'/files';return this;}});file_upload.Views.FileInput=Backbone.View.extend({initialize:function(options){_.bindAll(this,'fail','uploading','done');options=options||{};this.url=options.url||'/upload';this.data=options.data||{};this._name=options.name||'files[]';this._multiple=options.hasOwnProperty('multiple')?options.multiple:true;this._enabled=options.hasOwnProperty('enabled')?options.enabled:true;return this;},tagName:'input',events:{'change':'change'},render:function(){this.$el.attr('type','file').attr('name',this._name);if(this._multiple){this.$el.attr('multiple',true);}
